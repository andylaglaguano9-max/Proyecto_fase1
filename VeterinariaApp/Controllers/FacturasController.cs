...
namespace VeterinariaApp.Controllers
{
    public class FacturasController : Controller
    {
        ...
        public async Task<IActionResult> Index(string orden, string buscar, int pagina = 1)
        {
            ...
            var query = _context.Facturas.Where(e => e.Activo).AsQueryable();
        ...
        public async Task<IActionResult> Create(Factura entity)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, Factura entity)
        {
            if (id != entity.Id) return NotFound();

            if (ModelState.IsValid)
            {
                var original = await _context.Facturas.FindAsync(id);
                if (original != null)
                {
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            var entity = await _context.Facturas.FirstOrDefaultAsync(m => m.Id == id && m.Activo);
