...
namespace VeterinariaApp.Controllers
{
    public class CategoriasProductoController : Controller
    {
        ...
        public async Task<IActionResult> Index(string orden, string buscar, int pagina = 1)
        {
            ...
            var query = _context.CategoriasProducto.Where(e => e.Activo).AsQueryable();
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            var entity = await _context.CategoriasProducto.FindAsync(id);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, CategoriaProducto entity)
        {
            if (id != entity.Id) return NotFound();

            if (ModelState.IsValid)
            {
                var original = await _context.CategoriasProducto.FindAsync(id);
                if (original != null)
                {
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            var entity = await _context.CategoriasProducto.FirstOrDefaultAsync(m => m.Id == id && m.Activo);
