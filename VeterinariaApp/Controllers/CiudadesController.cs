...
namespace VeterinariaApp.Controllers
{
    public class CiudadesController : Controller
    {
        ...
        public async Task<IActionResult> Index(string orden, string buscar, int pagina = 1)
        {
            ...
            var query = _context.Ciudades.Where(e => e.Activo).AsQueryable();
        ...
        public async Task<IActionResult> Create(Ciudad entity)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, Ciudad entity)
        {
            if (id != entity.Id) return NotFound();

            if (ModelState.IsValid)
            {
                var original = await _context.Ciudades.FindAsync(id);
                if (original != null)
                {
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            var entity = await _context.Ciudades.FirstOrDefaultAsync(m => m.Id == id && m.Activo);
